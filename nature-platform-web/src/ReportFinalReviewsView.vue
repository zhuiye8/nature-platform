  ...
  <div class="page-shell page section-stack">
    <header class="page-header">
      <div class="page-title-group">
        <h2 class="page-title">报告最终审核</h2>
        <p class="page-subtitle">节点 15：审核人由节点规则槽位 FINAL_REVIEWER 自动确定。</p>
      </div>
      <el-space>
    ...
    <el-card class="tip-card np-info-strip">
      <el-alert
        ...
        :closable="false"
        show-icon
        title="本页用于查看最终审核状态，审核动作统一在“详情页”处理。"
      />
    </el-card>
    ...
    <el-card class="table-card">
      <el-table :data="rows" v-loading="loading" empty-text="暂无可处理项">
        ...
        <el-table-column prop="onSitePackageObjectKey" label="现场测评压缩包" min-width="260" show-overflow-tooltip>
...
/**
 * @input Report final review list API, workflow todo API, and router navigation helper
 * @output Node-15 final review board with status projection and detail-page handoff
 * @position Report final review page for viewing tasks and entering unified review detail page
 * @doc-sync Update this header and folder INDEX.md when this file changes.
 */
import { onMounted, ref } from "vue";
import { ElMessage } from "element-plus";
import { useRouter } from "vue-router";
import { fetchReportFinalReviews, type ReportFinalReviewRecord } from "./report-final-review-service";
...
function resolveTaskKey(row: ReportFinalReviewRecord) {
  if (row.taskId !== null && row.taskId !== undefined && String(row.taskId).trim().length > 0) {
    return `REPORT_FINAL_REVIEW:${row.taskId}`;
  }
...
async function loadRows() {
  ...
  try {
    const [listRows, todoRows] = await Promise.all([
      ...
      fetchTodoTasks({ type: "REPORT_FINAL_REVIEW" }).catch(() => [])
    ]);
    rows.value = listRows;
    const nextMap: Record<number, string> = {};
    for (const task of todoRows) {
      if (task.taskType === "REPORT_FINAL_REVIEW" && Number.isFinite(task.bizId)) {
...
function openDetail(row: ReportFinalReviewRecord) {
  ...
  void router.push(toTaskDetailPath("REPORT_FINAL_REVIEW", row.projectRegisterId, taskKey || undefined));
}
